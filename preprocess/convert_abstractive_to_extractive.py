...
def convert_to_extractive_driver(args):
    ...
    data_example_column, data_summarized_column = summarization_name_mapping[args.dataset]
    ...
    for name in tqdm(
        ...
        if args.dataset:  # if loading using the `nlp` library
            ...
            source_file = current_dataset[data_example_column]
            target_file = current_dataset[data_summarized_column]
        else:
        ...
        # Get predictions
        predictions_file = f'/nlp/projects/faithsum/{args.dataset}/{name}.out'
        with open(predictions_file, 'r') as fd:
            prediction_docs = fd.readlines()

        if args.max_examples is not None and args.max_examples < len(target_file):
            rand_idxs = np.sort(np.random.choice(range(len(target_file)), size=args.max_examples, replace=False))
            print(f'Sampling {args.max_examples} examples')

            source_file = [source_file[i] for i in rand_idxs]
            target_file = [target_file[i] for i in rand_idxs]
            prediction_docs = [prediction_docs[i] for i in rand_idxs]
        ...
        if args.shard_interval:  # if sharding is enabled
            ...
            convert_to_extractive_process(args, nlp, source_docs, target_docs, prediction_docs, name)
...
def convert_to_extractive_process(
    args, nlp, source_docs, target_docs, prediction_docs, name, piece_idx=None
):
    """
    ...
    prediction_docs_tokenized = tokenize(
        nlp,
        prediction_docs,
        args.n_process,
        args.batch_size,
        name=(" " + name + "-" + args.prediction_ext),
        tokenizer_log_interval=args.tokenizer_log_interval,
    )
    ...
    t0 = time()
    for (preprocessed_data, target_doc, rouge_results) in pool.map(
        _example_processor,
        zip(source_docs_tokenized, target_docs_tokenized, prediction_docs_tokenized),
    ):
    ...
    results_fn = os.path.expanduser('~/rouge_oracle_results.csv')
    print(f'Saving ROUGE results to {results_fn}')
    results.to_csv(results_fn, index=False)
    cols = list(results.columns)
    if 'selection' in cols:
        print(results['selection'].value_counts())
    for col in cols:
        if col == 'selection':
            continue
        print(f'{col}: {round(results[col].dropna().mean(), 3)}')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prediction_ext", type=str, default="prediction", help="extension of target files"
    )
    parser.add_argument(
        "--oracle_mode",
        type=str,
        default="gain",
        choices=['greedy', 'combination', 'gain', 'oracle'],
