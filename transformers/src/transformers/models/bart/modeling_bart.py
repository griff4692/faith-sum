...
class BartMert(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        ...
        self.extract_classifier = nn.Linear(config.d_model, 1)
    ...
    def sentence_mask(self, cls_mask, sent_idx_to_mask, prev_mask):
        """
    ...
    def forward(
        ...
        for batch_idx in range(batch_size):
            cls_h = last_hidden_state[batch_idx, cls_mask[batch_idx], :]
            sent_preds = self.extract_classifier(cls_h).squeeze(-1)
            extractor_logits.append(sent_preds)
            num_sents = cls_mask[batch_idx].sum()
            sent_masks = torch.cat([
                self.sentence_mask(cls_mask[batch_idx].unsqueeze(0), [sent_idx], attention_mask[batch_idx].unsqueeze(0))
                for sent_idx in range(num_sents)
            ], dim=0)
            batch_sent_masks.append(sent_masks)

            decoder_input_ids_rep = decoder_input_ids[batch_idx].unsqueeze(0).repeat(num_sents, 1)
            encoder_hidden_states = last_hidden_state[batch_idx].unsqueeze(0).repeat(num_sents, 1, 1)

            decoder_outputs = self.decoder(
                input_ids=decoder_input_ids_rep,
                attention_mask=decoder_attention_mask,
                encoder_hidden_states=encoder_hidden_states,
                encoder_attention_mask=sent_masks,
                head_mask=decoder_head_mask,
                cross_attn_head_mask=cross_attn_head_mask,
                past_key_values=past_key_values,
                inputs_embeds=decoder_inputs_embeds,
